...
import (
	sap_api_caller "sap-api-integrations-classification-reads-rmq-kube/SAP_API_Caller"
	sap_api_input_reader "sap-api-integrations-classification-reads-rmq-kube/SAP_API_Input_Reader"
	"sap-api-integrations-classification-reads-rmq-kube/config"
...
func callProcess(caller *sap_api_caller.SAPAPICaller, msg rabbitmq.RabbitmqMessage) (err error) {
	...
	class := extractData(msg.Data())
	accepter := getAccepter(msg.Data())
	caller.AsyncGetClass(class, accepter)
	return nil
}

func extractData(data map[string]interface{}) (class string) {
	sdc := sap_api_input_reader.ConvertToSDC(data)
	class = sdc.Class.Class
	return
}
