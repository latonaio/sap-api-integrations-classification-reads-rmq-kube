...
import (
	...
	"io/ioutil"
	"net/http"
	sap_api_output_formatter "sap-api-integrations-classification-reads-rmq-kube/SAP_API_Output_Formatter"
	"strings"
	"sync"
...
func (c *SAPAPICaller) Class(class string) {
	...
	err = c.outputter.Send(c.outputQueues[0], map[string]interface{}{"message": classData, "function": "ClassificationClass"})
	if err != nil {
		c.log.Error(err)
		return
	}
	...
	err = c.outputter.Send(c.outputQueues[0], map[string]interface{}{"message": charcData, "function": "ClassificationCharacteristic"})
	if err != nil {
		c.log.Error(err)
		return
	}
	...
	err = c.outputter.Send(c.outputQueues[0], map[string]interface{}{"message": classDescriptionData, "function": "ClassificationClassDescription"})
	if err != nil {
		c.log.Error(err)
		return
	}
